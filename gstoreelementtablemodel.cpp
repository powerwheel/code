#include "gstoreelementtablemodel.h"
 ...
 *	  2.int element: 所属通道(1-6)
 * 返回值：
 * 	  无
 */
GStoreElementTableModel::GStoreElementTableModel(QObject *parent,int element) :
 ...
 * 	  int:行数
 */
int GStoreElementTableModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    return ROW_COUNT;
}
 ...
 * 	  int:列数
 */
int GStoreElementTableModel::columnCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return COLUMN_COUNT;
}
 ...
 * 	  QVariant:显示的数据
 */
QVariant GStoreElementTableModel::data(const QModelIndex &index, int role) const
{
    int row = index.row();
 ...
 * 	  bool:是否设置成功
 */
bool GStoreElementTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    int row = index.row();
    int column = index.column();
 ...
 */
Qt::ItemFlags GStoreElementTableModel::flags(const QModelIndex &index) const
{
    int row = index.row();
    int column = index.column();
 ...
 * 	  无
 */
void GStoreElementTableModel::selectAllItems(bool f)
{
 ...
 * 	  无
 */
void GStoreElementTableModel::initFromSetting()
{
 ...
 * 	  无
 */
void GStoreElementTableModel::init_data()
{
    for(int i = 0; i < ROW_COUNT; ++i){
    ...
    for(int i = 1; i <= 6; i++){
        if(i == m_element){
            m_table[1][i].setChecked(true);
        }else{
            m_table[1][i].setChecked(false);
        }
    }
    ...
    int cur_row = 4;
    m_table[cur_row][0].setText("Basic Item");
    int col = 1;
    ...
    for(int j = GpckDataBasicItem::ItemUthd; j <= GpckDataBasicItem::Item_hcf; ++j){
        m_table[cur_row][col].setType(GStoreTableType::Item);
        m_table[cur_row][col].setText(basic_item.getItemName(j));
        m_table[cur_row][col].setItemIndex(j);
        ++col;
        if(col == COLUMN_COUNT){
            cur_row++;
            col = 1;
    ...
    cur_row++;
    m_table[cur_row][0].setText("Hrm Item");

    col = 1;
    for(int i = GpckDataBasicItem::ItemU0; i <= GpckDataBasicItem::ItemPhdf0; ++i){
        m_table[cur_row][col].setType(GStoreTableType::Item);
        m_table[cur_row][col].setText(basic_item.getItemName(i));
        m_table[cur_row][col].setItemIndex(i);
        ++col;
    ...
    m_table[cur_row][0].setText("Wave Item");

    m_table[cur_row][1].setType(GStoreTableType::Item);
    m_table[cur_row][1].setText("U");
    m_table[cur_row][1].setItemIndex(GStoreTableType::ITEM_U);

    m_table[cur_row][2].setType(GStoreTableType::Item);
    m_table[cur_row][2].setText("I");
    m_table[cur_row][2].setItemIndex(GStoreTableType::ITEM_I);
